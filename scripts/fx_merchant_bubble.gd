...
## SEYYAR SATICININ güvenli bölge baloncuğu (traveling_merchant.gd _create_protection_bubble).
##
## Kullanıcı isteği (2026-09-25): "seyyar satıcının koruma baloncuğu şovalye adamın ESKİ koruma baloncuğu olsun. ve bunun
## içindeyken de dışardaki hava durumunu sesleri vb tıpkı şovalye adamın bariyerinin içindeykenki gibi boğuk ve daha az
## duyalım." -> Şovalye'nin R kubbesi aynı gün sprite sayfalarına geçmişti (fx_paladin_barrier.gd, satıcı onun "gold"
## varyantını kullanıyordu). Bu dosya Şovalye'nin ESKİ (shader tabanlı) baloncuğunun birebir kopyası (git 95df7ef
## scripts/fx_paladin_barrier.gd): shaders/shield_dome.gdshader + fx_paladin_overlay.gd çatlak/parça katmanı, eski
## Şovalye rengiyle. Boğuk ses ortak bileşenden (dome_sound_muffle.gd - Şovalye kubbesiyle AYNI davranış).
##
## z_index MUTLAKA absolute (z_as_relative = false) - göreceli negatif bir değer zemin TileMapLayer'larının arkasında kalırdı.

var radius: float = 126.0 ## traveling_merchant.gd GameManager.MERCHANT_ZONE_RADIUS ile add_child'dan ÖNCE üzerine yazar
...
func _ready() -> void:
	...
	## İçerideyken dışarıdaki sesler boğuk (Şovalye kubbesiyle aynı ortak bileşen).
	var muffle := Node2D.new()
	muffle.set_script(load("res://scripts/dome_sound_muffle.gd"))
