...
4. **Karakter nitelikleri ve geliştirme kartları çalışır durumda:**
   - Can (max_health)
   - Hareket hızı (speed)
   - **Zırh (armor)** — yeni eklendi, gelen hasarı azaltır (ama en az
     1 hasar her zaman işler, tamamen zırhla dokunulmaz olamazsın)
   - Hasar (silahın mermi hasarı)
   - Ateş hızı (ne sıklıkla ateş ettiği)
   
   Seviye atladığında bu 5 nitelikten rastgele 3'ü kart olarak çıkıyor,
