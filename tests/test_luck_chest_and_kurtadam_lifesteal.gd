...
## 2) Kurt Adam'ın pasif can çalma oranı %0.3'den %1'e çıkmalı.
##
...
func test_kurtadam_passive_lifesteal_is_one_percent() -> void:
	var def: Dictionary = Characters.get_def(6) ## 6 = Kurt Adam (bkz. characters.gd)
	assert(def.get("name", "") == "Kurt Adam", "id=6 Kurt Adam olmali, bulunan: %s" % def.get("name"))
	assert(is_equal_approx(float(def.get("lifesteal", 0.0)), 0.01),
		"Kurt Adam pasif can calma orani 0.01 (%%1) olmali, bulunan: %s" % def.get("lifesteal"))
	assert(def.get("passive", "").find("%1") != -1,
		"Kurt Adam pasif aciklamasi %%1 icermeli, bulunan: %s" % def.get("passive"))
